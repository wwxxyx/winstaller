...
afc_error_t AFCFileRefOpen(struct afc_connection *conn, const char *path, unsigned
    int mode, unsigned int unknown, afc_file_ref *ref);
...
__CFString * AMDeviceCopyDeviceIdentifier(struct am_device *device);

mach_error_t AMDShutdownNotificationProxy(void *);

/*edits by geohot*/
mach_error_t AMDeviceDeactivate(struct am_device *device);
mach_error_t AMDeviceActivate(struct am_device *device, CFDictionaryRef);
/*end*/
...
//F t_performOperation _performOperation = (t_performOperation)0x3c39fa4b;
/* ----------------------------------------------------------------------------
